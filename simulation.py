...
def simulate(stage: int, state: float, 
             cost_func_arr: List[Callable], num_sim: int, pick_control: bool=False, training: bool=True) -> Dict[Tuple[float, int], float]:
    """
    ...
    for _ in range(num_sim):
        ...
        for i in iter_range:
            ...
            if pick_control:
                ...
                if training:
                    ...
                    if np.random.rand() < epsilon:
                        ...
                        sol = minimize_scalar(lambda control: arbitrage_cost(i, control, load, solar) +
                                                cost_func_arr[i+1](next_state(i, i_state, control, irr, load)),
                                            bounds=(lb, ub), method="bounded")
                        opt_control = np.clip(sol.x + np.random.normal(0, 0.5), lb, ub)
                else:
                    opt_control = sol.x
            else:
            ...
            # if pick_control:
            #     print(f"solar: {solar},load: {load},control: {opt_control},state: {i_state},cost: {cost}")
        ...
        if pick_control and not training:
...
def carbon_emission_cost(stage, control, load, solar, city):
    if city == "Phoenix":
        CI = [420, 410, 400, 390, 400, 410, 430, 410, 350, 250, 180, 150, 
              140, 145, 160, 210, 290, 380, 460, 470, 450, 440, 430, 425]
    elif city == "Sacramento":
        CI = [340, 330, 320, 310, 320, 330, 350, 340, 310, 280, 260, 240,
              230, 235, 250, 280, 320, 370, 410, 400, 380, 370, 360, 350]
    elif city == "Seattle":
        CI = [120, 110, 100, 100, 100, 110, 120, 130, 140, 135, 130, 125,
              120, 120, 125, 130, 140, 150, 160, 150, 140, 130, 125, 120]
    else:
        raise ValueError("City not recognized. Please use 'Phoenix', 'Sacramento', or 'Seattle'.")
    
    p_grid = load - solar + control
    return p_grid * CI[stage] * (1 if p_grid > 0 else 0)
...
if __name__ == "__main__":
    data = simulate(0, 25, [dumb_cost for i in range(STAGE)], 5000)

    models = NN_Linefit.backward_pass(data)
    for i in range(10):
        data = simulate(0, 25, models, 1, pick_control=True)
        models = NN_Linefit.backward_pass(data)
