...
def _solve(stage: int, state: float):
    ...
    for next in state_space:
        ...
        if control is None: # control is inadmissible
            continue
        irr_range, _ = get_irr_and_load_range(stage, CITY)
        min_sol = irr_range[0] * N_SOLAR * AREA_SOLAR * SOL_EFFICIENCY
        max_sol = irr_range[1] * N_SOLAR * AREA_SOLAR * SOL_EFFICIENCY
        if control > min_sol: # state transition is non deterministic
            worst_next = next_state(state, min_sol)
            best_next = next_state(state, max_sol)

            # all reachable next states (irrespective of chosen control)
            all_next = [state for state in state_space if worst_next <= state <= best_next]
            n_real = len(all_next)
            if n_real == 0:
                all_next = [max(state for state in state_space if state < worst_next)]
                n_real = 1
            # all reachable next states with given control
            possible_next = [state for state in state_space if worst_next <= state <= min(next,best_next)]
            possible_next.extend([next]*(n_real-len(possible_next)))

            cost_sum = 0
            for p_next in possible_next:
                next_controls, next_cost = solve(stage+1, p_next)
...
if __name__ == "__main__":
    ...
    filename = CITY + '_' + STRUCTURE + '_' + str(N_BATT) + '_' + str(N_SOLAR) + '_NOGRID_' + '_policy.pkl'
