...
def _solve(stage: int, state: float, parameters: Parameters):
    ...
    dist = uniform(loc=Plmean*0.8, scale=Plmean*1.2 - Plmean*0.8)
    ...
    for next in parameters.state_space:
        if next < get_grid_down_energy_threshold(stage+1, parameters):  # if next state is in the range of the distribution
...
if __name__ == "__main__":
    params = Parameters(N_BATT=5,N_SOLAR=20,CITY='Phoenix',STRUCTURE='A',MAX_STAGE=24)  # Create a parameters instance
